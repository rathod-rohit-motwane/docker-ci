        ...
        if data is not None:
            ...
            if mac_address.count(':') != 5:
                ...
                sqlite_fifo.push_data(
                    cursor_log_to_cloud, conn_log_to_cloud, log_to_cloud_table, data)
                ...
                if uuid_prefix == None or uuid_suffix == None:
                    #mac address is not found in the uuid map file, so add it to the log_server db
                    #append mac_id not found in data 
                    print("uuidprefix suffix null")
                    data=mac_address+"mac_not_found"
                    sqlite_fifo.push_data(
                    cursor_log_to_cloud, conn_log_to_cloud, log_to_cloud_table, data)
                    sqlite_fifo.pop_data(cursor_json,conn_json,json_table_name)
                else:    
                    ...
                    try:
                        ...
                        print("error in json")
                        #add the data as a messge to the log_server db 

                        sqlite_fifo.pop_data(cursor_json,conn_json,json_table_name)
                        data=mac_address+"uuid_config_error"
                        sqlite_fifo.push_data(
                    cursor_log_to_cloud, conn_log_to_cloud, log_to_cloud_table, data)
                    ...
                    if server_response==200:
                        ...
                        print("server not responding, backingup the data")
                        time.sleep(0.000005)
        else:
            time.sleep(0.000005)
